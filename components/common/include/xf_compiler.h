...
void xf_read_once_size(const volatile void *p, void *res, int size)
{
    switch (size) {
...
static __always_inline
void xf_write_once_size(volatile void *p, void *res, int size)
{
    switch (size) {
...
#define XF_READ_ONCE(x) \
    ({ \
        union { \
            _typeof_(x) __val; \
            char __c[1]; \
        } __u = { .__c = { 0 } }; \
        xf_read_once_size(&(x), __u.__c, sizeof(x)); \
        __u.__val; \
    })

#define XF_WRITE_ONCE(x, val) \
    ({ \
        union { \
            _typeof_(x) __val; \
            char __c[1]; \
        } __u = { .__val = (val) }; \
        xf_write_once_size(&(x), __u.__c, sizeof(x)); \
        __u.__val; \
    })
