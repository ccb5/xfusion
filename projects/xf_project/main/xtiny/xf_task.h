 ...
 *
 * @param _p_task       需要销毁的任务。 @ref xf_task_t* .
 * @param _cb_func      任务函数。 @ref xf_task_cb_t.
 * @param _user_data    任务内的用户数据。 @ref xf_task_t.user_data.
 * @return xf_err_t
 ...
 *      - XF_OK                 成功
 */
#define xf_task_destroy(_p_task)          xf_task_destroy_i(_p_task)
 ...
 * @param _user_data    任务内的用户数据。 @ref xf_task_t.user_data.
 * @param _arg          传给任务的参数。
 * @param[out] _pp_task 传出的任务句柄。 @ref xf_task_t** .
 *                      同时作为临时变量使用，因此必须传入。
 ...
 */
#define xf_task_start(_cb_func, _user_data, _arg, _pp_task) \
                                        xf_task_start_i((_cb_func), (_user_data), (_arg), (_pp_task))
 ...
 *
 * @param _p_task       要运行的任务。 @ref xf_task_t* .
 * @param _arg          传给任务的参数。
 * @return xf_task_async_t 任务状态。
 *      - XF_TASK_READY        就绪
 *      - XF_TASK_BLOCKED      阻塞
 *      - XF_TASK_TERMINATED   结束
 */
#define xf_task_run(_p_task, _arg)        xf_task_run_i((_p_task), (_arg))
 ...
 * @brief 获取任务状态.
 *
 * @param _p_task       任务句柄。 @ref xf_task_t* .
 * @return xf_task_async_t @ref xf_task_async_t
 *      - XF_TASK_READY        就绪
 *      - XF_TASK_BLOCKED      阻塞
 *      - XF_TASK_TERMINATED   结束
 */
#define xf_task_get_state(_p_task)      xf_task_attr_get_state(_p_task)
 ...
 * @param _p_task       任务句柄。 @ref xf_task_t* .
 */
#define xf_task_set_ready(_p_task)      xf_task_attr_set_state((_p_task), XF_TASK_READY)
 ...
 * @param _p_task       任务句柄。 @ref xf_task_t* .
 */
#define xf_task_set_blocked(_p_task)    xf_task_attr_set_state((_p_task), XF_TASK_BLOCKED)
 ...
 * @param[out] _p_xf_err    传出错误码。填入 NULL 时不传出。 @ref xf_err_t* .
 *                          - XF_ERR_TIMEOUT     超时
 *                          - XF_OK              成功
 * @param[out] _p_e_msg     传出事件消息。填入 NULL 时不传出。 @ref xf_event_msg_t* .
 */
#define xf_task_wait_until(_me, _id, _tick, _p_xf_err, _p_e_msg) \
                                        xf_task_wait_until_i((_me), (_id), (_tick), (_p_xf_err), (_p_e_msg))
 ...
 * @param[out] _p_e_msg     传出事件消息。填入 NULL 时不传出。 @ref xf_event_msg_t* .
 */
#define xf_task_wait_until_ms(_me, _id, _ms, _p_xf_err, _p_e_msg) \
                                        xf_task_wait_until_ms_i((_me), (_id), (_ms), (_p_xf_err), (_p_e_msg))
